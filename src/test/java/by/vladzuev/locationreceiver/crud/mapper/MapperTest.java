...
public final class MapperTest extends AbstractSpringBootTest {
    ...
    public void lazySourceShouldBeMapped() {
        try (final MockedStatic<HibernateUtil> mockedHibernateUtil = mockStatic(HibernateUtil.class)) {
            final AddressEntity givenSource = new AddressEntity(255L);
            mockedHibernateUtil.when(() -> HibernateUtil.isFetched(same(givenSource))).thenReturn(true);
    ...
    public void lazySourceShouldBeMappedToNull() {
        try (final MockedStatic<HibernateUtil> mockedHibernateUtil = mockStatic(HibernateUtil.class)) {
            final AddressEntity givenSource = new AddressEntity(255L);
            mockedHibernateUtil.when(() -> HibernateUtil.isFetched(same(givenSource))).thenReturn(false);
