...
import by.vladzuev.locationreceiver.kafka.property.KafkaProperty;
...
public final class AvroGenericRecordSerializerTest extends AbstractSpringBootTest {
    ...
    private static final Map<String, ?> GIVEN_CONFIGURATION_PROPERTIES = Map.of(KafkaProperty.SCHEMA.getName(), GIVEN_SCHEMA);
    ...
    private static DatumWriter<GenericRecord> findDatumWriter(final AvroGenericRecordSerializer serializer) {
        return (DatumWriter<GenericRecord>) ReflectionUtil.getProperty(serializer, FIELD_NAME_DATUM_WRITER, DatumWriter.class);
    }

    private static Schema findSchema(final DatumWriter<GenericRecord> datumWriter) {
        return ReflectionUtil.getProperty(datumWriter, FIELD_NAME_SCHEMA, Schema.class);
    }
